...
| 2026-07-04 | Inventory Integrity, Adjustments, and Fulfillment | ✅ DONE | `vendor/bin/pint --dirty --format agent`; `php artisan wayfinder:generate --with-form --no-interaction`; `php artisan test --compact tests/Feature/Inventory/BookingAvailabilityTest.php tests/Feature/Inventory/HandoverVerificationTest.php tests/Feature/Inventory/RequisitionIssuanceTest.php tests/Feature/Inventory/StockAdjustmentWorkflowTest.php` (29 passed); `npx eslint resources/js/pages/inventory/movements/Index.vue resources/js/pages/inventory/requisitions/Show.vue resources/js/actions/App/Http/Controllers/Inventory/StockMovementController.ts resources/js/actions/App/Http/Controllers/Inventory/RequisitionController.ts resources/js/routes/inventory/movements/index.ts resources/js/routes/inventory/requisitions/index.ts` (warnings only: ignored generated files, existing single-word Vue page names) | Added shared asset-integrity enforcement across booking approval and handover initiation/verification, admin stock adjustment and cycle-count workflows with reason-coded audit history and lot-safe variance handling, plus partial requisition fulfillment/backorder support with line-level remaining quantities and updated operator UI/status behavior. |
