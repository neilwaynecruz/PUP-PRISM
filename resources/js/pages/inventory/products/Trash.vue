...
function confirmRestore(): void {
    ...
    router.put(
        `/inventory/products/${selectedProduct.value.id}/restore`,
        {},
        {
            onSuccess: () => {
...
function confirmBulkForceDelete(): void {
    router.post(
        '/inventory/products/bulk-force-delete',
        {
            ids: Array.from(selectedIds.value),
        },
        {
    ...
    <div class="flex flex-col gap-6 p-4 sm:p-6">
        ...
        <div class="rounded-xl border border-border/50 bg-card shadow-sm">
            <div class="border-b border-border/40 px-5 py-4">
                ...
                <div class="text-xs text-muted-foreground/80">Narrow deleted products by date range, deleter, or keyword.</div>
            </div>
        ...
        <div class="rounded-xl border border-border/50 bg-card shadow-sm">
            <div class="flex items-center justify-between border-b border-border/40 px-5 py-4">
                <div class="text-sm font-semibold tracking-tight">Deleted products</div>
                <span class="inline-flex items-center rounded-full border border-border/40 bg-muted/40 px-2.5 py-1 text-[11px] font-medium text-muted-foreground">{{ products.data.length }} total</span>
            </div>
            ...
            <DialogContent>
                <DialogHeader class="space-y-3">
                    <DialogTitle class="text-rose-600"
                        >Permanently delete product?</DialogTitle
                    >
                    <DialogDescription>
                        This will
                        ...
                        >
                        The product and all associated records will be gone
                        forever.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter class="gap-2">
        ...
        <Dialog v-model:open="bulkRestoreDialogOpen">
            <DialogContent>
                <DialogHeader class="space-y-3">
                    ...
                    <DialogDescription>
                        This will restore
                        <strong>{{ selectedIds.size }} product(s)</strong> to
                        the active products list.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter class="gap-2">
                    <DialogClose as-child>
        ...
        <Dialog v-model:open="bulkForceDeleteDialogOpen">
            <DialogContent>
                <DialogHeader class="space-y-3">
                    <DialogTitle class="text-rose-600"
                        >Permanently delete
                        {{ selectedIds.size }} products?</DialogTitle
                    >
                    <DialogDescription>
                        This will
                        <strong class="text-rose-600"
                            >permanently delete</strong
                        >
                        <strong>{{ selectedIds.size }} product(s)</strong>.
                        <br /><br />
                        <span class="font-medium text-rose-600"
                            >This action cannot be undone.</span
                        >
                        These products and all associated records will be gone
                        forever.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter class="gap-2">
