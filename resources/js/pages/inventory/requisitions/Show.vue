...
const workflowSteps = computed(() => {
    ...
    return [
        ...
        {
            key: 'approved',
            title: currentStatus === 'Rejected' ? 'Approval skipped' : 'Approved',
            completed: props.requisition.approved_at !== null,
            current: currentStatus === 'Approved',
            timestamp: props.requisition.approved_at,
            actor: props.requisition.approver?.name ?? (currentStatus === 'Rejected' ? 'Rejected before approval' : 'Pending approval'),
        ...
        },
        {
            ...
            actor: props.requisition.issuer?.name ?? (currentStatus === 'Rejected' ? 'Not issued' : 'Pending issuance'),
    ...
    <div class="flex flex-col gap-6 p-4 sm:p-6" data-testid="requisition-show-page">
        <div v-if="isDeleted" class="rounded-xl border border-rose-200 bg-rose-50 p-4 dark:border-rose-900 dark:bg-rose-950">
            <div class="flex items-center gap-3">
                ...
                <span class="font-medium text-rose-900 dark:text-rose-100">This requisition has been deleted and is in trash.</span>
                <Button variant="ghost" size="sm" as-child class="ml-auto">
                    <Link href="/inventory/requisitions/trash">Go to Trash</Link>
                </Button>
            </div>
        </div>
            ...
            <div class="mb-4 flex items-center justify-between gap-3">
                <div>
                    ...
                    <div class="text-sm text-muted-foreground">
                        Submitted, approved, and issued milestones update as the requisition moves through the workflow.
                    </div>
                </div>
                <div class="text-sm text-muted-foreground">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                >
                    <div class="flex items-center gap-3">
                        ...
                        >
                            {{ step.completed ? '✓' : step.key === 'submitted' ? '1' : step.key === 'approved' ? '2' : '3' }}
                        </span>
                        <div>
                ...
                >
                    ...
                    <div class="mt-3 grid gap-2">
                        ...
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span :data-testid="`requisition-line-issued-${l.sku ?? l.id}`">{{ l.qty_issued }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <Dialog v-if="can.reject" v-model:open="rejectDialogOpen">
                ...
                <DialogContent>
                    <DialogHeader class="space-y-3">
                        ...
                        <DialogDescription>
                            This will permanently reject requisition #{{ requisition.id }}. The requester will be notified.
                        </DialogDescription>
                    </DialogHeader>
                    <Form
                    ...
                    >
                        <div class="grid gap-2">
                            ...
                            <Input id="reject_notes" name="notes" placeholder="Explain why this request is being rejected" required />
                            <InputError :message="errors.notes" />
                        </div>
                        <DialogFooter class="gap-2">
                            ...
                            <Button type="submit" :disabled="processing" variant="destructive">Confirm reject</Button>
                        </DialogFooter>
                    </Form>
                </DialogContent>
            </Dialog>
