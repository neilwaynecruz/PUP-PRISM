...
const props = defineProps<{
    ...
    categories: { data: Row[]; links: PaginationLink[] };
    can: { create: boolean };
}>();

defineOptions({
    layout: { breadcrumbs: [{ title: 'Admin', href: index() }, { title: 'Categories', href: index() }] },
});
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4 sm:p-6">
        <div class="flex items-start justify-between gap-3">
            <Heading variant="small" title="Categories" description="Product classification reference data." />
        ...
        <div class="overflow-x-auto rounded-xl border border-border/60 bg-card shadow-sm">
            <table class="w-full min-w-[800px] table-fixed text-sm">
                ...
                <tbody class="divide-y divide-border/60">
                    <tr v-for="row in categories.data" :key="row.id" class="transition-colors hover:bg-muted/30 [&>td]:px-4 [&>td]:py-3.5">
        ...
        <div v-if="categories.links.length" class="flex flex-wrap justify-center gap-1 mt-4">
            <Button v-for="(link, i) in categories.links" :key="i" variant="ghost" size="sm" as-child :disabled="!link.url" :class="link.active ? 'bg-primary/10 text-primary' : ''">
