...
const props = defineProps<{ filters: { search: string; active: boolean | null }; origins: { data: Row[]; links: { url: string | null; label: string; active: boolean }[] }; can: { create: boolean } }>();
defineOptions({ layout: { breadcrumbs: [{ title: 'Admin', href: index() }, { title: 'Origins', href: index() }] } });
...
<template>
    ...
    <div class="flex flex-col gap-6 p-4 sm:p-6">
        <div class="flex items-start justify-between gap-3"><Heading variant="small" title="Origins" description="Product origin reference data." /><Button v-if="can.create" as-child size="sm"><Link :href="create()">New origin</Link></Button></div>
        ...
        <div class="overflow-x-auto rounded-xl border border-border/60 bg-card shadow-sm"><table class="min-w-full text-sm"><thead class="bg-muted/40 text-xs uppercase text-muted-foreground [&>th]:px-4 [&>th]:py-3"><tr><th>Name</th><th>Products</th><th>Status</th><th class="text-right">Actions</th></tr></thead><tbody class="divide-y divide-border/60"><tr v-for="row in origins.data" :key="row.id" class="[&>td]:px-4 [&>td]:py-3"><td class="font-medium">{{ row.name }}</td><td>{{ row.products_count }}</td><td>{{ row.is_active ? 'Active' : 'Inactive' }}</td><td class="text-right"><Button variant="ghost" size="sm" as-child><Link :href="edit(row.id)">Edit</Link></Button></td></tr></tbody></table></div>
    </div>
</template>
